...
import { OllamaAgent } from '../worker';
import { QueenAgent } from '../queenAgent';
import { SubQueenAgent } from '../subQueenAgent';
...
jest.mock('path', () => ({
  ...
  sep: '/', // Define sep as a property of the mocked object
...
describe('Orchestrator and Agent Communication', () => {
  ...
  beforeEach(() => {
    ...
    // Set mock implementations for fs/promises
    (fs.mkdir as jest.Mock).mockImplementation(() => Promise.resolve());
    (fs.writeFile as jest.Mock).mockImplementation(() => Promise.resolve());

    // Set mock implementations for ollama
    (ollama.chat as jest.Mock).mockImplementation(() => Promise.resolve({ message: { content: 'Mocked Ollama Response' } }));
    (ollama.list as jest.Mock).mockImplementation(() => Promise.resolve({ models: [{ name: 'llama3' }, { name: 'codellama' }] }));

    orchestrator = new Orchestrator();
    orchestrator.setProjectFolder(null); // Ensure project folder is null by default for tests
  ...
  it('should handle unknown receiver IDs', async () => {
    ...
    await orchestrator.dispatchMessage(message);
    expect(consoleErrorSpy).toHaveBeenCalledWith('Agent with ID unknown-receiver not found.');
  });
  ...
  describe('OllamaAgent File Saving', () => {
    it('should save the generated code to a file if specified in the prompt', async () => {
      ...
      const ollamaAgent = orchestrator['agents'].get('ollama-agent-1') as OllamaAgent;
      ...
      // Ensure projectFolderPath is set for this test
      ollamaAgent['projectFolderPath'] = mockProjectFolder;
    ...
    it('should not save the file if project folder is not set', async () => {
      ...
      // Directly set projectFolderPath to null for this specific agent instance
      ollamaAgent['projectFolderPath'] = null;
    ...
    it('should not save the file if save instruction is not in prompt', async () => {
      ...
      // Directly set projectFolderPath for this specific agent instance
      ollamaAgent['projectFolderPath'] = mockProjectFolder;
